...
class Control {
    ...
    update(vx,vy,m,jf) {
        ...
        if (this._bomb.isPressed) { 
            ...
            //         new THREE.MeshPhongMaterial( {color:0x6a6a6a,map: new THREE.TextureLoader().load("assets/textures/zuniga.jpg")}));
            //         bomb.position.set(players.p1.element.position.x,players.p1.element.position.y,players.p1.element.position.z);  
            //         scene.add(bomb);
            //         for (let i = 0; i == 4; i++) {
            //             scene.remove(bomb);
            //             visible = false;
    ...
    pressBomb() {
        ...
        if(contBomb == 0){
            classBomb.crearBomba();
            setTimeout(function(){ classBomb.eliminarBomba(); }, 2000);
            contBomb += 1;
        }else{
            classBomb.crearBomba();
        }
